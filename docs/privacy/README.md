...
```
https://averycorp.github.io/prismTask/privacy/
```

## GitHub Pages enablement (one-time)

1. Go to [repo Settings → Pages](https://github.com/averycorp/prismTask/settings/pages).
2. **Source:** "Deploy from a branch."
3. **Branch:** `main`.
4. **Folder:** `/docs`.
5. Click **Save**. GitHub Pages enables and the site becomes available at `https://averycorp.github.io/prismTask/` within ~1 minute. The privacy policy lands at `/privacy/`.

The repo is public (verified via `gh repo view averycorp/prismTask --json isPrivate` → `false`), so GitHub Pages is free — no Pro / Team tier required.

## Plugging into Play Console

Play Console → Store presence → Store listing → **Privacy policy**: paste `https://averycorp.github.io/prismTask/privacy/` and save.
...
1. In GoDaddy (or wherever `prismtask.app` is registered), add a DNS CNAME from `privacy` to `averycorp.github.io`.
