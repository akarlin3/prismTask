...
class CoachingViewModel @Inject constructor(
    ...
    fun checkPerfectionism(
        ...
        if (!coachingRepository.shouldShowPerfectionismCard(
                editCount, rescheduleCount, subtasksAdded, subtasksCompleted
            )
        ) return
    ...
    fun onTaskCompleted(
        ...
        if (!coachingRepository.shouldCelebrate(
                completedSubtaskCount, totalSubtaskCount, daysOverdue, firstAfterGap
            )
        ) return
    ...
    fun shouldSuggestBreakdown(task: TaskEntity, subtaskCount: Int): Boolean {
        return coachingRepository.shouldSuggestBreakdown(task, subtaskCount)
    }
}
