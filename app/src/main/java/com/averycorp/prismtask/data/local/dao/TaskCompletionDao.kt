...
interface TaskCompletionDao {
    ...
    @Query("SELECT completed_date AS date, COUNT(*) AS count FROM task_completions WHERE completed_date >= :startDate AND completed_date <= :endDate GROUP BY completed_date ORDER BY completed_date ASC")
    fun getCompletionCountByDate(startDate: Long, endDate: Long): Flow<List<DateCount>>

    @Query("SELECT CAST(strftime('%w', completed_date / 1000, 'unixepoch', 'localtime') AS INTEGER) AS dayOfWeek, COUNT(*) AS count FROM task_completions WHERE completed_date >= :startDate AND completed_date <= :endDate GROUP BY dayOfWeek")
    fun getCompletionCountByDayOfWeek(startDate: Long, endDate: Long): Flow<List<DayOfWeekCount>>

    @Query("SELECT CAST(strftime('%H', completed_at_time / 1000, 'unixepoch', 'localtime') AS INTEGER) AS hour, COUNT(*) AS count FROM task_completions WHERE completed_date >= :startDate AND completed_date <= :endDate GROUP BY hour")
    ...
    @Query("SELECT AVG(days_to_complete) FROM task_completions WHERE days_to_complete IS NOT NULL AND completed_date >= :startDate AND completed_date <= :endDate")
    fun getAverageDaysToComplete(startDate: Long, endDate: Long): Flow<Double?>

    @Query("SELECT CAST(SUM(CASE WHEN was_overdue = 1 THEN 1 ELSE 0 END) AS REAL) / CAST(COUNT(*) AS REAL) * 100.0 FROM task_completions WHERE completed_date >= :startDate AND completed_date <= :endDate")
