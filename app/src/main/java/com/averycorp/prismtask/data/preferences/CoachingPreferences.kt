 ...
 */
@Singleton
class CoachingPreferences @Inject constructor(
     ...
     */
    suspend fun getRemainingBreakdowns(): Int {
        return (FREE_BREAKDOWN_DAILY_LIMIT - getBreakdownCountToday()).coerceAtLeast(0)
    }
     ...
     */
    suspend fun hasReachedBreakdownLimit(): Boolean {
        return getBreakdownCountToday() >= FREE_BREAKDOWN_DAILY_LIMIT
    }
    ...
    suspend fun getLastAppOpen(): Long = context.dataStore.data.map { prefs ->
    ...
    suspend fun getDaysSinceLastOpen(): Int? {
        ...
        return java.util.concurrent.TimeUnit.MILLISECONDS.toDays(elapsed).toInt()
    }
     ...
     */
    suspend fun shouldShowEnergyCheckIn(): Boolean {
        return getTodayEnergyLevel() == null
    }
